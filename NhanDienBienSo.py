...
def recognized_plate(list_char_on_plate, size):
    ...
    print("So bien so: " + final_plate)
    print("Thoi gian nhan dien: " + str(time.time() - t0))

cap = cv2.VideoCapture('test_videos/NDBS06.MOV') # video path
...
if __name__=="__main__":
    while(cap.isOpened()):
        ...
        # out.write(_frame)
        cv2.imshow('DO AN XU LI ANH VA UNG DUNG', _frame)
        ...
        if possible_plates is not None:
            ...
            if (distance < 100):
                if(countPlates < countPlates_threshold):
                    cv2.imshow('Bien So', possible_plates[0])
                    temp = []
